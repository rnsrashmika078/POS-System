...
type ProductStore = {
  ...
  editProduct: (product: Food, newVal: { count: number; name: string }) => void;
...
export const useProductStore = create<ProductStore>((set) => ({
  ...
  addProduct: (product) =>
    set((state) => {
      const exist = state.products.some((item) => item.name === product.name);
  ...
  removeProduct: (product) => {
    set((state) => {
      const exist = state.counter.some((item) => item.name === product.name);
      ...
      if (exist) {
        return {
          ...
          counter: state.counter.filter((item) => item.name !== product.name),
        };
      }
      return {
